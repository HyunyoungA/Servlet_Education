...
<%
	...
	if(!myInfo.getInterest().equals("-")){
		String[] splitStr = myInfo.getInterest().split(", ");
	...
	//등산, 요리, 기타: null/checked/null/checked/null/checked
		...
		<form action="<%= request.getContextPath() %>/update.me" method="post" id="updateForm" name="updateForm">
			<table>
				<tr>
					...
					<td width="200px"><input type="text" name="myId" style="background:lightgray;" readonly value="<%= myInfo.getUserId() %>"></td>
				</tr>
				<tr>
					<td><label class="must">*</label> 이름</td>
					<td><input type="text" name="myName" required value="<%= myInfo.getUserName() %>"></td>
				</tr>
				<tr>
					<td><label class="must">*</label> 닉네임</td>
					<td><input type="text" maxlength="15" name="myNickName" required value="<%= myInfo.getNickName() %>"></td>
					<td><input type="button" id="nickCheck" onclick="checkNick();" value="중복확인"></td>
				</tr>
				<tr>
					<td>연락처</td>
					<td>
						<input type="tel" maxlength="11" name="myPhone" placeholder="(-없이)01012345678" value="<%= myInfo.getPhone().equals("-") ? "" : myInfo.getPhone() %>">
					</td>
				</tr>
				<tr>
					<td>이메일</td>
